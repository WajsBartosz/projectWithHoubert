...
logout;
    ...
    <div id='main'>
        ...
        <?php 
            if(!isset($_SESSION['login'])) 
                echo "Zaloguj się aby dodać temat.";
            else{?>
                <form action='./php/scripts/addPost.php' method='post'>
                    <input type='numer' name='sectionID' value="<?php echo $_GET['section'] ?>" hidden>
                    <div class="input-group mb-3">
                      <span class="input-group-text input-span-width" id="basic-addon1">Temat</span>
                      <input type="text" class="form-control" placeholder="Maks. 50 znaków" aria-label="Username" aria-describedby="basic-addon1" required name='postTopic'>
                    </div>
                    <div class="input-group">
                      <span class="input-group-text input-span-width">Treść</span>
                      <textarea class="form-control" aria-label="With textarea" required name='postContent'></textarea>
                    </div>             
                    <center><button type="submit" class="btn btn-primary btn-submit-form" name='sendbutton' value='1'>Dodaj post</button></center>
                  </form>
        <?php } ?>
        </div>
        <hr>
        <div class='display-posts-section'>
          <?php require_once("./php/scripts/displayAllPosts.php") ?>
        </div>
    </div>
